...
cdef class py_Graph:
    ...
    def __cinit__(self,*arg):
        ...
        if len(arg)==0:
            ...
            nodes_weight = np.array([x for x in arg[4]], dtype=np.double)

            self.reshape_Graph(_num_nodes, _num_edges,  edges_from,  edges_to, nodes_weight)
        else:
            print('Error：py_Graph类为被成功初始化，因为提供参数数目不匹配，参数个数为0或5。')
    ...
    def total_nodes_weight(self):
        return deref(self.inner_graph).total_nodes_weight

    @property
    def nodes_weight(self):
        return deref(self.inner_graph).nodes_weight
    ...
    cdef reshape_Graph(self, int _num_nodes, int _num_edges, int[:] edges_from, int[:] edges_to,double[:] nodes_weight):
        cdef int *cint_edges_from = <int*>malloc(_num_edges*sizeof(int))
        cdef int *cint_edges_to = <int*>malloc(_num_edges*sizeof(int))
        cdef double *cdoube_nodes_weight = <double*>malloc(_num_nodes*sizeof(double))
        cdef int i
        for i in range(_num_nodes):
            cdoube_nodes_weight[i] = nodes_weight[i]
        for i in range(_num_edges):
            cint_edges_from[i] = edges_from[i]
        for i in range(_num_edges):
            cint_edges_to[i] = edges_to[i]
        self.inner_graph = shared_ptr[Graph](new Graph(_num_nodes,_num_edges,&cint_edges_from[0],&cint_edges_to[0],&cdoube_nodes_weight[0]))
        free(cint_edges_from)
        free(cint_edges_to)

    def reshape(self,int _num_nodes, int _num_edges, int[:] edges_from, int[:] edges_to,double[:] nodes_weight):
        self.reshape_Graph(_num_nodes, _num_edges, edges_from, edges_to,nodes_weight)
...
cdef class py_GSet:
    ...
    cdef G2P(self,Graph graph):
        ...
        return py_Graph(num_nodes,num_edges,cint_edges_from,cint_edges_to,cdouble_nodes_weight)
